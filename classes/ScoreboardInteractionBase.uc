...
var() localized string HealthText, AdminText, NetText, KillsText, PlayerText, PingText, PLText, ReadyText, NotReadyText, FPH, TimeLimit, FooterText, Restart;
...
function drawStats(Canvas canvas, int i, PlayerReplicationInfo pri, float YL, int PlayerBoxSizeY, int BoxSpaceY, int BoxTextOffsetY, int BoxXPos, int BoxWidth) {
    canvas.DrawColor = HUDClass.default.WhiteColor;
    canvas.StrLen(pri.Kills, KillWidthX, YL);
...
function PostRender(Canvas canvas) {
    ...
    foreach ViewportOwner.Actor.DynamicActors(class'PlayerReplicationInfo', pri) {
    ...
    for ( i = 0; i < PlayerCount; i++ ) {
        ...
        if ( KFPlayerReplicationInfo(priArray[i])!=None && KFPlayerReplicationInfo(priArray[i]).ClientVeteranSkill != none ) {
            VeterancyBox = KFPlayerReplicationInfo(priArray[i]).ClientVeteranSkill.default.OnHUDIcon;
    ...
    /** Draw Custom stats */
    for (i = 0; i < PlayerCount; i++) {
        drawStats(canvas, i, priArray[i], YL, PlayerBoxSizeY, BoxSpaceY, BoxTextOffsetY, BoxXPos, BoxWidth);
    }
